...
Upstream dependencies are placed into `Puppetfile`. This is porbably another file you are going to change, adding the public module you decide to use.
...
The psick module can manage classification and it does it in phased way entirely configurable via Hiera. Four phases are available:
  ...
  - **pre**, prerequisites classes, they are applied in a normal catalog run (that is, always except in the very first Puppet run, if firstrun is enabled) before all the other classes. Here for example you can include classes that manage netwrok, repositories, and everything you want to apply first.
...
Strictly speaking, besides the ordering, which you can decide to use or not, there's no difference among classes inclused in the pre, base or profiles phases.
...
Infrastructure data is tyically site specific, so we placed in a separated module the sample Hieradata used in the PSICK control repo, to ease the usage of custom data in custom modules (or directly in the control repo).
